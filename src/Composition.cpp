...
{
    BOOL keepCandidateInput = IsCandidateListActive();
    DWORD_PTR keyLen = _pCompositionProcessorEngine ? _pCompositionProcessorEngine->GetVirtualKeyLength() : 0;

    DIME_DEBUG_LOG(L"keepCandidate=%d keyLen=%llu isOurComp=%d hasPresenter=%d",
        keepCandidateInput ? 1 : 0,
        keyLen,
        (_pComposition == pComposition) ? 1 : 0,
    ...
    if (keepCandidateInput && pContext)
    {
        DIME_DEBUG_LOG(L"branch=keepCandidateRefresh");
        // Notepad may terminate empty composition; keep encoding in candidate window only.
        _RefreshActiveCandidateUI(ecWrite, pContext);
        pContext->Release();
        return S_OK;
    }

    DIME_DEBUG_LOG(L"branch=fullTeardown");
...
BOOL CDIME::IsCandidateListActive()
{
    return _pCompositionProcessorEngine != nullptr &&
...
void CDIME::_OnCompositionStarted(TfEditCookie ec, _In_ ITfContext *pContext)
{
...
HRESULT CDIME::_RemoveDummyCompositionForComposing(TfEditCookie ec, _In_ ITfComposition *pComposition)
{
    HRESULT hr = S_OK;
