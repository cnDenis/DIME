...
void CBaseWindow::_Move(int x, int y, BOOL fRedraw)
{
    if (_wndHandle != nullptr)
    {
        ...
        SetWindowPos(_wndHandle, 0, x, y, 0, 0, flags);
    }
    else
    {
...
void CBaseWindow::_Resize(int x, int y, int cx, int cy, BOOL fRepaint)
{
    if (_wndHandle != nullptr)
    {
        MoveWindow(_wndHandle, x, y, cx, cy, fRepaint ? TRUE : FALSE);
    }
    else
    {
...
void CBaseWindow::_RepaintNow()
{
    if (_wndHandle != nullptr)
    {
        RedrawWindow(_wndHandle, nullptr, nullptr, RDW_INVALIDATE | RDW_UPDATENOW | RDW_ALLCHILDREN | RDW_ERASE);
    }
}
