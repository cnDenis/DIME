        ...
        isSelectionChanged)
    {
        ...
        if (_IsComposing())
        {
            ...
            if (SUCCEEDED(_pComposition->GetRange(&pRangeComposition)))
            {
                ...
                {
                    BOOL active = IsCandidateListActive();
                    DIME_DEBUG_LOG(L"selectionOutsideComposition active=%d composing=%d",
                        active ? 1 : 0,
                        _IsComposing() ? 1 : 0);
                    if (!active)
                    {
                        DIME_DEBUG_LOG(L"EndComposition due to selection change");
