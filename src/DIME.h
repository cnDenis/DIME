...
class CDIME : public ITfTextInputProcessorEx,
...
public:
    ...
    HRESULT _StartCompositionAt(TfEditCookie ec, _In_ ITfContext *pContext);
    void _TerminateComposition(TfEditCookie ec, _In_ ITfContext *pContext, BOOL isCalledFromDeactivate = FALSE);
    void _TerminateCompositionCancel(TfEditCookie ec, _In_ ITfContext *pContext);
    void _SaveCompositionContext(_In_ ITfContext *pContext);
    void _OnCompositionStarted(TfEditCookie ec, _In_ ITfContext *pContext);
    HRESULT _RefreshActiveCandidateUI(TfEditCookie ec, _In_ ITfContext *pContext);
    void _RefreshCandidateInputModeStatus();

    // Clear any in-progress composition / candidate list so a toggle of a
    // persistent mode (e.g. Ctrl+M "only common characters") starts from a
    // clean slate instead of leaving a stale candidate list on screen.
    void _ResetInputForModeChange();

    // Floating status bar (small bar showing input-mode states; clickable).
    ...
    BOOL IsCandidateListActive();

    // comless helpers
    static HRESULT CDIME::CreateInstance(REFCLSID rclsid, REFIID riid, _Outptr_result_maybenull_ LPVOID* ppv, _Out_opt_ HINSTANCE* phInst, BOOL isComLessMode);
    static HRESULT CDIME::ComLessCreateInstance(REFGUID rclsid, REFIID riid, _Outptr_result_maybenull_ void **ppv, _Out_opt_ HINSTANCE *phInst);
    static HRESULT CDIME::GetComModuleName(REFGUID rclsid, _Out_writes_(cchPath)WCHAR* wchPath, DWORD cchPath);
...
private:
    ...
    void _CancelComposition(_In_opt_ ITfContext *pContext);
    BOOL _IsComposing();
    ...
    // 若候选为 CMD:ENG: 进入临时英文并保留编码, 返回 TRUE (已处理, 勿上屏).
    BOOL _TryApplyCmdEng(_In_ const CStringRange *pCandidate, TfEditCookie ec, _In_ ITfContext *pContext);
...
private:
    ...
    CStatusWindow *_pStatusWindow;
    BOOL _isThreadFocused : 1;
